...
class HeltecV3SensorManager : public SensorManager {
  ...
  #ifdef DISPLAY_CLASS
    HeltecV3SSD1306Display* _display;
  #endif
...
public:
  ...
  #ifdef DISPLAY_CLASS
  void setDisplay(HeltecV3SSD1306Display* display) {
    _display = display;
    if (_display) {
      _display->setBrightness(_board->screen_brightness);
    }
  }
  #endif
  ...
  bool setSettingValue(const char* name, const char* value) override {
    ...
    bool result = _board->setSettingValue(name, value);
    
    // If screen setting was changed, actually control the display
    #ifdef DISPLAY_CLASS
    if (result && strcmp(name, "screen") == 0 && _display) {
      if (_board->screen_enabled) {
        _display->turnOn();
        _display->setBrightness(_board->screen_brightness);
      } else {
        _display->turnOff();
    ...
    if (result && strcmp(name, "brightness") == 0 && _display) {
      _display->setBrightness(_board->screen_brightness);
    }
    #endif
