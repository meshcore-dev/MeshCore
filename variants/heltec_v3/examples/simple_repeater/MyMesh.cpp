  ...
  if (memcmp(command, "setperm ", 8) == 0) {   // format:  setperm {pubkey-hex} {permissions-int8}
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (strcmp(name, "screen") == 0) {
        screen_found = true;
        screen_status = (strcmp(sensors.getSettingValue(i), "1") == 0) ? "ON" : "OFF";
    ...
    if (sensors.setSettingValue("screen", "1")) {
    ...
    if (sensors.setSettingValue("screen", "0")) {
    ...
    const char* val = sensors.getSettingByKey("brightness");
    if (val) {
      sprintf(reply, "Brightness: %s", val);
    } else {
      strcpy(reply, "Err - Brightness setting not available");
    }
  } else if (strncmp(command, "brightness ", 11) == 0) {
    const char* val = command + 11;
    if (sensors.setSettingValue("brightness", val)) {
