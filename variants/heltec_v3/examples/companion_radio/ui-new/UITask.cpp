      ...
      if (!_in_settings_submenu) {
        ...
        if (_sensors != NULL) {
          ...
          for (int i = 0; i < num; i++) {
            ...
            if (strcmp(name, "screen") == 0) {
              screen_enabled = (strcmp(_sensors->getSettingValue(i), "1") == 0);
        ...
        if (_brightness_popup) {
          ...
          for (uint8_t i = 0; i < 4; i++) {
            display.setCursor(box_x + 6, py);
            if (_brightness_popup_item == i) display.print(">");
            if (i == 0) display.print("Back");
            if (i == 1) display.print("Low");
            if (i == 2) display.print("Medium");
            if (i == 3) display.print("High");
            if (_brightness_popup_item == i) display.print("<");
            py += 10;
          }
        }
  ...
  bool handleInput(char c) override {
    ...
    if (c == KEY_NEXT || c == KEY_RIGHT) {
      ...
      if (_page == HomePage::SETTINGS && _in_settings_submenu) {
        ...
        _display_menu_item = (_display_menu_item + 1) % 4;  // cycle through 0 (back), 1 (screen), 2 (led), 3 (brightness)
        return true;
      }
#endif
      ...
      if (_page == HomePage::SETTINGS) {
        _in_settings_submenu = false;  // reset submenu state when entering page
        _display_menu_item = 0;  // reset menu selection
      }
#endif
      return true;
    }
...
void UITask::begin(DisplayDriver* display, SensorManager* sensors, NodePrefs* node_prefs) {
  ...
  if (_display != NULL) {
    ...
    if (_sensors != NULL) {
      int num = _sensors->getNumSettings();
      for (int i = 0; i < num; i++) {
        if (strcmp(_sensors->getSettingName(i), "brightness") == 0) {
          int value = atoi(_sensors->getSettingValue(i));
          if (value < 0) value = 0;
          if (value > 255) value = 255;
          ((HeltecV3SSD1306Display*)_display)->setBrightness(value);
          break;
        }
      }
    }
...
void UITask::newMsg(uint8_t path_len, const char* from_name, const char* text, int msgcount) {
  ...
  if (_display != NULL) {
    ...
    if (_sensors != NULL) {
      int num = _sensors->getNumSettings();
      for (int i = 0; i < num; i++) {
        if (strcmp(_sensors->getSettingName(i), "screen") == 0) {
          screen_enabled = (strcmp(_sensors->getSettingValue(i), "1") == 0);
          break;
        }
      }
          ...
          for (int i = 0; i < num; i++) {
            if (strcmp(_sensors->getSettingName(i), "screen") == 0) {
              if (strcmp(_sensors->getSettingValue(i), "0") == 0) {
                _display->turnOff();  // Turn off display after alert shown
              }
              break;
            }
          }
...
char UITask::handleDoubleClick(char c) {
  ...
  if (_sensors != NULL) {
    ...
    for (int i = 0; i < num; i++) {
      if (strcmp(_sensors->getSettingName(i), "screen") == 0) {
        bool screen_enabled = (strcmp(_sensors->getSettingValue(i), "1") == 0);
        if (!screen_enabled) {
          // Screen is disabled - turn it back on
          _sensors->setSettingValue("screen", "1");
          if (_display != NULL) {
            _display->turnOn();
          }
...
char UITask::handleTripleClick(char c) {
  ...
  if (_display != NULL && !_display->isOn()) {
    // Force screen on by setting the flag and turning on display
    if (_sensors != NULL) {
      _sensors->setSettingValue("screen", "1");
    }
    _display->turnOn();
...
void UITask::toggleScreen() {
  if (_sensors != NULL) {
    ...
    for (int i = 0; i < num; i++) {
      if (strcmp(_sensors->getSettingName(i), "screen") == 0) {
        if (strcmp(_sensors->getSettingValue(i), "1") == 0) {
          // Turn off the screen setting (this will call display->turnOff())
          _sensors->setSettingValue("screen", "0");
          // Immediately turn display back on temporarily to show the alert
          if (_display) {
            _display->turnOn();
          }
          notify(UIEventType::ack);
...
void UITask::setBrightnessLevel(int level) {
  if (_sensors != NULL) {
    ...
    char buf[6];
    sprintf(buf, "%d", value);
    _sensors->setSettingValue("brightness", buf);
    if (_display != NULL) {
      ((HeltecV3SSD1306Display*)_display)->setBrightness(value);
    }
    char msg[20];
