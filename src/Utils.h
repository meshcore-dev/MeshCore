...
public:
  ...
  /**
   * \brief  calculates the SHA256 hash of two fragments, 'frag1' and 'frag2' (in that order), storing in
   * 'hash' and truncating.
   */
  ...
  /**
   * \brief  Encrypts the 'src' bytes using AES128 cipher, using 'shared_secret' as key, with key length fixed
   * at CIPHER_KEY_SIZE. Final block is padded with zero bytes before encrypt. Result stored in 'dest'.
  ...
  /**
   * \brief  Decrypt the 'src' bytes using AES128 cipher, using 'shared_secret' as key, with key length fixed
   * at CIPHER_KEY_SIZE. 'src_len' should be multiple of block size, as returned by 'encrypt()'. \returns  The
   * length in bytes put into 'dest'. (dest may contain trailing zero bytes in final block)
   */
