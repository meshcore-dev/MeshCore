...
DispatcherAction Mesh::onRecvPacket(Packet *pkt) {
  ...
  if (pkt->isRouteDirect() && pkt->getPayloadType() == PAYLOAD_TYPE_TRACE) {
    if (pkt->path_len < MAX_PATH_SIZE) {
      ...
      if (offset >= len) { // TRACE has reached end of given path
        ...
        return ACTION_RETRANSMIT_DELAYED(5,
                                         d); // schedule with priority 5 (for now), maybe make configurable?
      }
    }
    return ACTION_RELEASE;
  }
  ...
  if (pkt->isRouteDirect() && pkt->path_len >= PATH_HASH_SIZE) {
    ...
    return ACTION_RELEASE; // this node is NOT the next hop (OR this packet has already been forwarded), so
                           // discard.
  }
  ...
  default:
    MESH_DEBUG_PRINTLN("%s Mesh::onRecvPacket(): unknown payload type, header: %d", getLogDateTime(),
                       (int)pkt->header);
...
DispatcherAction Mesh::routeRecvPacket(Packet *packet) {
  if (packet->isRouteFlood() && !packet->isMarkedDoNotRetransmit() &&
    ...
    return ACTION_RETRANSMIT_DELAYED(packet->path_len,
                                     d); // give priority to closer sources, than ones further away
  }
  return ACTION_RELEASE;
}
...
Packet *Mesh::createAdvert(const LocalIdentity &id, const uint8_t *app_data, size_t app_data_len) {
  ...
  len += SIGNATURE_SIZE; // will fill this in later
...

  {
    ...
    memcpy(&message[msg_len], id.pub_key, PUB_KEY_SIZE);
    msg_len += PUB_KEY_SIZE;
    memcpy(&message[msg_len], &emitted_timestamp, 4);
    msg_len += 4;
    memcpy(&message[msg_len], app_data, app_data_len);
    msg_len += app_data_len;
