...
void BaseChatMesh::onPeerDataRecv(mesh::Packet *packet, uint8_t type, int sender_idx, const uint8_t *secret,
  ...
  if (type == PAYLOAD_TYPE_TXT_MSG && len > 5) {
    ...
    if (flags == TXT_TYPE_PLAIN) {
      ...
      uint32_t ack_hash; // calc truncated hash of the message timestamp + text + OUR pub_key, to prove to
                         // sender that we got it
      mesh::Utils::sha256((uint8_t *)&ack_hash, 4, data, 9 + strlen((char *)&data[9]), self_id.pub_key,
                          PUB_KEY_SIZE);
...
bool BaseChatMesh::onPeerPathRecv(mesh::Packet *packet, int sender_idx, const uint8_t *secret, uint8_t *path,
...
bool BaseChatMesh::onContactPathRecv(ContactInfo &from, uint8_t *in_path, uint8_t in_path_len,
  ...
  // NOTE: default impl, we just replace the current 'out_path' regardless, whenever sender sends us a new
...
void BaseChatMesh::onAckRecv(mesh::Packet *packet, uint32_t ack_crc) {
  ...
  if ((from = processAck((uint8_t *)&ack_crc)) != NULL) {
    ...
    if (packet->isRouteFlood() && from->out_path_len >= 0) {
      // we have direct path, but other node is still sending flood, so maybe they didn't receive reciprocal
      // path properly(?)
      handleReturnPathRetry(*from, packet->path, packet->path_len);
    }
  }
...
void BaseChatMesh::handleReturnPathRetry(const ContactInfo &contact, const uint8_t *path, uint8_t path_len) {
  ...
  mesh::Packet *rpath =
      createPathReturn(contact.id, contact.getSharedSecret(self_id), path, path_len, 0, NULL, 0);
...
int BaseChatMesh::sendCommandData(const ContactInfo &recipient, uint32_t timestamp, uint8_t attempt,
  ...
  auto pkt = createDatagram(PAYLOAD_TYPE_TXT_MSG, recipient.id, recipient.getSharedSecret(self_id), temp,
                            5 + text_len);
  if (pkt == NULL) return MSG_SEND_FAILED;
...
bool BaseChatMesh::sendGroupMessage(uint32_t timestamp, mesh::GroupChannel &channel, const char *sender_name,
...
int BaseChatMesh::sendLogin(const ContactInfo &recipient, const char *password, uint32_t &est_timeout) {
  mesh::Packet *pkt;
  {
    ...
    if (recipient.type == ADV_TYPE_ROOM) {
      ...
      if (len > 15) len = 15; // max 15 chars currently
    ...
    pkt = createAnonDatagram(PAYLOAD_TYPE_ANON_REQ, self_id, recipient.id, recipient.getSharedSecret(self_id),
                             temp, tlen);
  }
  if (pkt) {
    uint32_t t = _radio->getEstAirtimeFor(pkt->getRawLength());
...
int BaseChatMesh::sendAnonReq(const ContactInfo &recipient, const uint8_t *data, uint8_t len, uint32_t &tag,
...
int BaseChatMesh::sendRequest(const ContactInfo &recipient, const uint8_t *req_data, uint8_t data_len,
  ...
  mesh::Packet *pkt;
  {
    ...
    pkt = createDatagram(PAYLOAD_TYPE_REQ, recipient.id, recipient.getSharedSecret(self_id), temp,
                         4 + data_len);
  }
  if (pkt) {
    uint32_t t = _radio->getEstAirtimeFor(pkt->getRawLength());
...
int BaseChatMesh::sendRequest(const ContactInfo &recipient, uint8_t req_type, uint32_t &tag,
                              uint32_t &est_timeout) {
  mesh::Packet *pkt;
  {
    uint8_t temp[13];
