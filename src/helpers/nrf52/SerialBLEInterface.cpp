...
void SerialBLEInterface::onSecured(uint16_t connection_handle) {
  ...
  if (instance) {
    if (instance->isValidConnection(connection_handle, true)) {
      ...
      // Apple: "The product will not read or use the parameters in the Peripheral Preferred Connection
      // Parameters characteristic." So we explicitly set it here to make Android & Apple match
