...
bool CommonCLI::savePrefs(FILESYSTEM* fs) {
  ...
  bool success = saveConfigJsonAtomic(fs, *_prefs, "/prefs.json", "/.prefs.json.new",
                                      s_last_prefs_save_stage, sizeof(s_last_prefs_save_stage));
  ADMIN_DBG_MS("savePrefs: end ok=%d dt=%lu stage=%s lfs_err=%d", (int) success, millis() - t0,
               s_last_prefs_save_stage[0] ? s_last_prefs_save_stage : "-", fsLastErrGet());
  return success;
}
  ...
  if (memcmp(config, "dutycycle ", 10) == 0) {
    ...
    if (isValidName(&config[5])) {
      ...
      if (savePrefs()) {
        ...
        ADMIN_DBG_MS("set name: savePrefs failed stage=%s lfs=%d", s_last_prefs_save_stage[0] ? s_last_prefs_save_stage : "-",
                     fsLastErrGet());
        formatPrefsSaveErr(reply);
      }
    } else {
