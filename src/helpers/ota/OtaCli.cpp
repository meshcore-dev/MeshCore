...
bool handle_ota_command(const char* command, char* reply, mesh::MainBoard& board) {
  ...
  if (is_cmd(a, "help|?|h", &rest)) {
    snprintf(reply, 160,
      ...
      "announce | self | folder | cache | config | key. Try `ota ls`.");
  ...
  // ---- inventory dashboard: running fw (self), the one fetch session, serving state ----
    ...
    if (n < 150) n += snprintf(reply + n, 160 - n, " | cache:%s", c.cache_active ? "on" : "off");
#endif
  ...
  // ---- broadcast beacon so peers discover our served motas (folder / cache / post-fetch re-seed).
    ...
    if (c.serving || c.manager.servedCount() > 0)
      ...
      strcpy(reply, "OK beacon sent (nothing to serve — folder/cache/captured delta)");
  ...
  } else if (is_cmd(a, "cache|seed|sd", &rest)) {
#if defined(OTA_SEEDER_CACHE)
    const char* p = rest;
    while (*p == ' ') p++;
    // `ota cache allow …` — runtime target filter (empty = all targets). Persisted via NodePrefs.
    if (strncmp(p, "allow", 5) == 0 && (p[5] == 0 || p[5] == ' ')) {
      const char* ap = p + 5;
      while (*ap == ' ') ap++;
      if (*ap == 0 || strncmp(ap, "list", 4) == 0) {
        if (c.cache_allow.allowAll()) {
          strcpy(reply, "cache allow: ALL targets (deltas only)");
        } else if (c.cache_allow.count() == 0) {
          strcpy(reply, "cache allow: NONE (empty filter)");
        } else {
          int n = snprintf(reply, 160, "cache allow (%u):", (unsigned)c.cache_allow.count());
          for (uint8_t i = 0; i < c.cache_allow.count() && n < 140; i++) {
            uint32_t tid = c.cache_allow.get(i);
        ...
        c.cache_allow.clear();
        c.config_dirty = true;
        if (c.cache_active) c.cache_seeder.refreshSource();
        strcpy(reply, "OK cache allow cleared — NONE (saved)");
      } else if (strncmp(ap, "reset", 5) == 0 || strncmp(ap, "defaults", 8) == 0) {
        c.cache_allow.reset();
        c.config_dirty = true;
        if (c.cache_active) c.cache_seeder.refreshSource();
        strcpy(reply, "OK cache allow reset — ALL targets (saved)");
        ...
        if (!tid) {
          strcpy(reply, is_add ? "ERR usage: ota cache allow add <env|hex>"
                               : "ERR usage: ota cache allow rm <env|hex>");
          return true;
        }
        if (is_add) {
          if (!c.cache_allow.add(tid)) { strcpy(reply, "ERR allowlist full (max 8)"); return true; }
          c.config_dirty = true;
          if (c.cache_active) c.cache_seeder.refreshSource();
          const char* env = ota_target_env_name(tid);
          snprintf(reply, 160, "OK allow +%s (saved)", env ? env : "target");
        } else {
          if (!c.cache_allow.remove(tid)) { strcpy(reply, "ERR not in allowlist"); return true; }
          c.config_dirty = true;
          if (c.cache_active) c.cache_seeder.refreshSource();
          strcpy(reply, "OK removed (saved)");
        }
      } else {
        strcpy(reply, "ERR usage: ota cache allow [list|add <env|hex>|rm <env|hex>|clear|reset]");
      }
      return true;
    }
    if (!c.cache_active) {
      snprintf(reply, 160, "ERR %s cache not active (mount failed at boot?)", OTA_SEEDER_MEDIA);
      return true;
    }
    OtaManager::FetchState fs = c.manager.fetchState();
    const char* pol = c.cache_allow.allowAll() ? "all"
                    : (c.cache_allow.count() == 0 ? "none" : "filter");
    if (c.cache_seeder.capturing() && fs != OtaManager::IDLE) {
      char midhx[9]; mesh::Utils::toHex(midhx, c.manager.fetchManifestId(), 4);
      unsigned have = (unsigned)c.manager.blocksHave(), tot = (unsigned)c.manager.blocksTotal();
      unsigned pct = tot ? (unsigned)((uint64_t)have * 100 / tot) : 0;
      snprintf(reply, 160, "%s cache | deltas/%s | files=%u %uK | capture %s %u/%u (%u%%) id=%s",
               OTA_SEEDER_MEDIA, pol,
               (unsigned)c.cache_seeder.fileCount(),
               (unsigned)(c.cache_seeder.totalBytes() / 1024),
               state_short(fs), have, tot, pct, midhx);
    } else {
      snprintf(reply, 160, "%s cache | deltas/%s | mounted | files=%u %uK | capture idle",
               OTA_SEEDER_MEDIA, pol,
               (unsigned)c.cache_seeder.fileCount(),
               (unsigned)(c.cache_seeder.totalBytes() / 1024));
    }
#else
    strcpy(reply, "ERR not built with OTA_SEEDER_CACHE");
#endif
...
#else
    strcpy(reply, "ERR self-serve disabled (removed v0.3.0; use folder/cache deltas)");
#endif
