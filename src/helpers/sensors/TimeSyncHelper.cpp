...
#ifdef ENV_INCLUDE_GPS // remove/extend when time can be synced from other sources
...
void TimeSyncHelper::process() {
    ...
    if (!_initialSyncCompleted) {
        ...
        if (sensors._location != nullptr) {
            sensors._location->syncTime();
            MESH_DEBUG_PRINTLN("GPS time sync requested via MicroNMEALocationProvider");
            
            while (sensors._location->waitingTimeSync()) {
                delay(100);
            }
            ...
            MESH_DEBUG_PRINTLN("GPS lock timeout - no valid GPS fix achieved");
            if (!_gpsWasOnBeforeSync) {
                sensors.stop_gps();
            }
            _initialSyncCompleted = true;
