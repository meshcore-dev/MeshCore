...
証明された920MHz帯の出力上限は **19mW（≈12.79dBm）** で、これは厳格な上限（丸め誤差の余地なし）とのことです。本ファームウェアはこれを踏まえ、T1000-Eの送信出力を **12dBm**（≈15.85mW）に制限しています。

アンテナは内蔵PCBアンテナ（利得 −0.45dBi）のみが証明対象で、物理的に交換できません。付属のまま使用してください。
...
ダイポールアンテナ（8.0dBi）は証明対象ではありますが、10dBmの出力設定と組み合わせるとEIRPが18dBmとなり、920MHz帯の上限16dBmを超えます（試験時は出力を約7.7dBmまで下げて8.0dBiと組ませることでEIRP15.7〜15.8dBmに収め、合格しています）。**Wio-SX1262搭載機では付属の低利得アンテナ（FPCまたはSpring、概ね3dBi以下）を使用してください。**
...
- **技適済み機器**：上記の一般原則より優先。認証時の出力・アンテナ構成の範囲内で使用してください（上記のT1000-E・Wio-SX1262搭載機の項を参照。Wio-SX1262は3dBi超のアンテナも条件付きで証明範囲内です）

高利得アンテナ（八木・指向性・ハイゲインコリニア等）は3dBiを超えるため使用できません。通常のホイップ/チップアンテナ（概ね0〜2dBi）の範囲で使ってください。

### 技適（工事設計認証）の確認方法

以下のリンクから、各機種の証明内容を直接確認できます。

| 機種 | 認証番号 | リンク |
|---|---|---|
| T1000-E | 217-252754 | [Seeed証明書PDF](https://files.seeedstudio.com/Seeed_Certificate/documents_certificate/114993369-TELEC.pdf) |
| Wio-SX1262搭載機 | 201-250230 | [総務省データベース](https://www.tele.soumu.go.jp/giteki/SearchServlet?pageID=jg01_01&PC=201&TC=N&PK=1&FN=250515N201&SN=%E8%AA%8D%E8%A8%BC&LN=34&R1=*****&R2=*****) |
